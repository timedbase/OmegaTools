...
import { TokenCreated as TokenCreatedEvent } from "../generated/AntiBotTokenFactory/AntiBotTokenFactory"
import { Token, User, FactoryStats, GlobalStats, DailyStats } from "../generated/schema"

export function handleAntiBotTokenCreated(event: TokenCreatedEvent): void {
  ...
  token.tokenType = "antibot"
  token.name = event.params.name
  token.symbol = event.params.symbol
  token.decimals = 18
  token.totalSupply = event.params.totalSupply
  token.initialSupply = event.params.totalSupply
  token.creationFee = BigInt.fromI32(0) // Fee not in event, would need to query contract
  ...
  updateFactoryStats(event.address.toHexString(), "antibot", BigInt.fromI32(0), event.block.timestamp)

  // Update global stats
  updateGlobalStats("tokenCreated", BigInt.fromI32(1), BigInt.fromI32(0))
